...
**`config.yml` is Jekyll's main configuration file**, located in the site root directory and written in YAML format. It stores the site's basic settings, build options, plugin configurations, custom variables, and more. Jekyll automatically reads and applies these configurations during the build process.<br>
...
The current `_config.yml` of this blog is as above. New content may be added and updated from time to time in the future.<br>
